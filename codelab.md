...
├── Dockerfile              # Root container specification for Google Cloud Run deployment
├── Procfile                # Cloud Run / Buildpacks process entrypoint
├── requirements.txt        # Root Python dependencies for ADK and agents
...
│   ├── Dockerfile          # Sub-package Dockerfile reference
...
Your project dependencies in `requirements.txt` (and `weather_agent/requirements.txt`):
